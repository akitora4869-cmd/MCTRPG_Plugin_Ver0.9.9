...
public class DreamlandsGenerator extends ChunkGenerator {
    ...
    public void generateNoise(WorldInfo worldInfo,
        ...
        for (int lx = 0; lx < 16; lx++) {
            for (int lz = 0; lz < 16; lz++) {
                ...
                // 寒色・暗色寄りの環境色を得るためのバイオーム。
                for (int y = worldInfo.getMinHeight(); y < worldInfo.getMaxHeight(); y += 4) {
                    chunkData.setBiome(lx, y, lz, chooseBiome(surface, c, r));
                }
            }
        }
    }
    ...
    private Biome chooseBiome(int surface, double continental, double ridge) {
        if (surface < SEA_LEVEL) return Biome.DEEP_COLD_OCEAN;
        if (continental < -0.35) return Biome.SOUL_SAND_VALLEY;
        if (ridge > 0.58) return Biome.BASALT_DELTAS;
